...
Local control on macOS/Linux (no tray, no extra deps): `./compass-ctl.sh [start|stop|restart|status]`, or double-click the Finder launchers `Compass Start.command` / `Compass Stop.command` / `Compass Restart.command` / `Compass Status.command`. The script runs uvicorn detached (`nohup`), tracks it via `.compass.pid` + the listening port (so it also stops/relaunches a server it didn't start — e.g. one launched by hand), and writes to `compass.log`. **Restart after code changes** — uvicorn runs without `--reload`, so edits aren't live until a restart. This is the cross-platform counterpart to `compass_tray.py`.

Local secrets in dev: at import, `main.py` loads `.compass_secret_key`, `.xai_key`, `.xai_model`, `.admin_emails` from the project root into the matching env vars (`COMPASS_SECRET_KEY`, `XAI_API_KEY`, `XAI_MODEL`, `ADMIN_EMAILS`) **iff that env var is unset** — so `uvicorn main:app` run directly on any OS gets the same secrets the Windows tray injects. All gitignored. Prod (Heroku) ships none of these files and sets real config vars, so the loader is a no-op there; an explicitly-set env var always wins.
