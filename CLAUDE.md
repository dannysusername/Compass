...
- **PUSH — `POST /sync`** (tasks): body `{"changes": {"tasks": [{id?, client_id?, updated_at, ...fields}]}, "deletes": {"tasks": [id]}}`. Upserts are **newest-wins** (apply only if pushed `updated_at` ≥ server's, else keep server). No-id rows are created and their `client_id` → new server id is returned in `id_map`. Deletes hard-delete + write a `Tombstone`. Cross-user/unknown ids skipped; pushed `class_id`/`tag_id` are ownership-validated (`_validated_fk`) → foreign refs nulled. Field allow-list: `_TASK_PUSH_FIELDS`.
- **`Tombstone`** (`user_id`, `kind`, `item_id`, `deleted_at`): written on delete so removals propagate (a hard delete alone just makes the row vanish). The normal `POST /tasks/{id}/delete` route also writes one. Recurring exclude/end-after stay plain updates (they bump `updated_at`, so pull catches them — no tombstone).
