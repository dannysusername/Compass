...
### Browser extension
A Manifest V3 Chromium extension lives in `extension/`. It's a thin client over the existing FastAPI server: `popup.html` renders a quick-add form, `lib/api.js` wraps `fetch()` with `credentials: 'include'` so the user's existing Compass cookie rides along, `options.html` lets the user point the extension at any Compass URL (defaults to `http://localhost:8000`). Auth flow: user logs into Compass once in a browser tab → cookie is set on the Compass origin → extension popup uses that cookie via host-permissioned cross-origin fetch. CORS is wired up in `main.py` with `CORSMiddleware(allow_origin_regex=r"^chrome-extension://[a-p]{32}$", allow_credentials=True)` — extensions are the only cross-origin clients allowed. The extension itself stores nothing except the Compass URL in `chrome.storage.local`. Phase 1 (quick-add popup) is built; Phase 2+ (side panel for full Today/Week/Class views) is planned but not yet implemented.
