...
**PULL side built; PUSH + offline client are later slices.** `Class`, `Tag`, `Task`, `CalendarEvent` carry a nullable `updated_at` (`default_factory` on insert + `sa_column_kwargs={"onupdate": ...}` so any ORM update bumps it — bulk `update()` statements bypass it, so touch `updated_at` explicitly there if you add one). It's **Optional on purpose** so the column adds cleanly to populated prod tables; the lifespan backfills NULLs (`updated_at = created_at` or now) so existing rows are syncable. `GET /sync?since=<ISO>` (`require_login`) returns the user's `classes`/`tags`/`tasks`/`events` changed strictly after `since` (everything when omitted) plus a `server_time` the client stores and passes back next pull. **Filtering is done in Python, not SQL**, to dodge SQLite/Postgres naive-vs-aware datetime comparisons (matches the collectors). No deletes yet (tombstones are a later slice) and no PUSH yet. Guard: `tests/test_sync.py` (incl. the load-bearing "an edit bumps updated_at → it syncs"). Roadmap/decisions live in the `project_local_first_roadmap` memory.
