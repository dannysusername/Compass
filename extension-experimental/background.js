...
async function postQuickTask(title, notes) {
    ...
    const token = await compassToken();
    ...
    const r = await fetch(base + "/tasks", {
        ...
        headers: {
            ...
            // Same per-user bearer token the side panel stores at login.
...
async function compassToken() {
    const { compass_token } = await chrome.storage.local.get("compass_token");
    return compass_token || null;
}
